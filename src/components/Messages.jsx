...
const Messages = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto mt-10 p-6 bg-white rounded shadow space-y-4">
      ...
      <div
        className="max-h-60 overflow-y-auto border p-3 space-y-2 rounded"
        ref={scrollRef}
      >
        {thread.map((r) => (
          <div key={r.id} className="border p-2 rounded bg-gray-50">
            <p className="text-sm text-gray-700 font-medium">@{r.sender.username}</p>
            <p>{r.content}</p>
          </div>
        ))}
      </div>
